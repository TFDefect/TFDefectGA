...
class DeltaMetricsExtractor(BaseMetricsExtractor):
    ...
    def get_previous_metrics(self, modified_blocks: Dict[str, List[str]]) -> Dict[str, dict]:
        ...
        en utilisant output.json si disponible.

        Args:
            modified_blocks (dict): Blocs Terraform modifiés.
        ...
        # Essayer de charger les métriques existantes depuis output.json
        cached_metrics = {}
        if os.path.exists(output_json_path):
            try:
                logger.info("Chargement des métriques depuis output.json...")
                with open(output_json_path, 'r') as f:
                    cached_metrics = json.load(f)
            except Exception as e:
                logger.warning(f"Impossible de charger output.json: {e}")
        
        for file in modified_blocks.keys():
            # Vérifier si les métriques pour ce fichier sont déjà dans output.json
            if file in cached_metrics and "data" in cached_metrics[file]:
                logger.info(f"Utilisation des métriques existantes pour {file} depuis output.json")
                previous_metrics[file] = cached_metrics[file]
                continue
            
            # Si non disponible, recalculer les métriques
            logger.info(f"Calcul des métriques pour {file} (non trouvé dans output.json)")
